            ...
            for _, Value in ipairs(self.Values) do
                if Matches(Value) then
                    ...
                    Button.InputBegan:Connect(function(Input)
                        ...
                        if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                            -- Mouse: no scroll-vs-tap ambiguity, toggle immediately.
                            ToggleValue();
                            LastDragValue = Value;
                            DraggingSelection = self.Multi and self.DragSelect == true;
                        elseif Input.UserInputType == Enum.UserInputType.Touch then
                            -- Touch: a press might be the start of a scroll gesture through
                            -- this button, not a tap. Wait to see if the finger moves before
                            -- deciding; only toggle on release if it stayed roughly in place.
