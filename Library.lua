...
function Library:Unload()
    ...
    if Library._OnUnload then
        pcall(Library._OnUnload)
    end

    if Library._MobileGui then
        pcall(function() Library._MobileGui:Destroy() end)
...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local function ValueText(Value)
            ...
            if self._DisplayValues and self._DisplayValues[Value] ~= nil then
                DisplayValue = self._DisplayValues[Value]
            end
            if Info.FormatListValue and type(Info.FormatListValue) == 'function' then
                local Ok, Result = pcall(Info.FormatListValue, DisplayValue);
            ...
            for _, Value in ipairs(self.Values) do
                if Matches(Value) then
                    ...
                    local function BeginDragSelection(Input)
                        ...
                        ChangedConn = InputService.InputChanged:Connect(function(Change)
                            local IsTouchMove = Input.UserInputType == Enum.UserInputType.Touch and Change == Input
                            local IsMouseMove = Input.UserInputType == Enum.UserInputType.MouseButton1
                                and Change.UserInputType == Enum.UserInputType.MouseMovement
...
function BaseGroupbox:AddPriorityDropdown(Idx, Info)
    ...
    function dropdown:Deselect(Value, Silent)
        local NewOrder = {}
        for _, v in ipairs(self.Order or {}) do
            if v ~= Value then
        ...
        if Silent then
            local selected = {}
            for _, v in ipairs(NewOrder) do selected[v] = true end
            self.Order = NewOrder
            self.Value = selected
            self:Display()
            self:RefreshButtons()
    ...
    DragFrame.InputBegan:Connect(function(Input)
        ...
        changedConn = InputService.InputChanged:Connect(function(Change)
            ...
            if Change.UserInputType == Enum.UserInputType.Touch then
                if Change ~= activeInput then return end
