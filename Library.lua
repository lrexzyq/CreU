    ...
    Instance.InputBegan:Connect(function(Input)
        ...
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            ...
            EndedConn = InputService.InputEnded:Connect(function(EndInput)
                ...
                -- would stop/disconnect this drag and could bleed input through
                -- to whatever was under that other touch.
...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local ListScale = Library:Create('UIScale', {
            Scale = (Library._MobileScale and Library._MobileScale.Scale) or Library.DPIScale or 1;
            Parent = ListOuter;
        });
        if Library._MobileScale then
            Library._MobileScale:GetPropertyChangedSignal('Scale'):Connect(function()
                ListScale.Scale = Library._MobileScale.Scale;
            end)
        end
...
do
    ...
    -- Default watermark text so the watermark is never blank/invisible out
    -- of the box. Library:SetWatermark isn't defined yet at this point in
    -- the file (it's declared further down in the same chunk), so the
    -- initial text/size is set directly here; callers can still override
    -- anytime afterward with SetWatermark/AddWatermark.
    do
...
function Library:SetWatermarkVisibility(Bool)
    if Library.Watermark then
        Library.Watermark.Visible = Bool == true;
    end
end;
...
function Library:MakeResizable(UI, DragFrame, Callback, MinSize)
    ...
    DragFrame.InputBegan:Connect(function(Input)
        ...
        changedConn = InputService.InputChanged:Connect(function(Change)
            ...
            if Change ~= activeInput and Change.UserInputType ~= Enum.UserInputType.MouseMovement then return end
            local Delta = Change.Position - dragStart
