...
do
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        function Slider:Display()
            ...
            local X = math.ceil(math.clamp(Alpha, 0, 1) * Slider.MaxSize);
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:BuildDropdownList()
            ...
            local Height = math.min(math.max(1, VisibleCount), self.MaxVisibleItems) * self.ItemHeight + SearchHeight + 2;
            ListOuter.Size = UDim2.fromOffset(math.max(120, DropdownOuter.AbsoluteSize.X), Height);
        end
        ...
        function Dropdown:SetSearchable(Value)
            Value = Value == true;
            if Value == self.Searchable then return end
            -- Search UI is created at construction to keep runtime mutation safe.
...
function Library:SetWatermarkSegments(Segments)
    ...
    for _, child in next, InnerFrame:GetChildren() do
        if child:IsA('TextLabel') then child:Destroy() end
    end
    local x = 5
    for i, segment in ipairs(Segments) do
        local text = type(segment) == 'function' and tostring(segment()) or tostring(segment)
        local label = Library:CreateLabel({
            Position = UDim2.fromOffset(x, 0),
            Size = UDim2.fromOffset(0, InnerFrame.AbsoluteSize.Y),
        ...
        x += label.TextBounds.X + 8
    end
    Library.Watermark.Size = UDim2.fromOffset(math.max(100, x + 5), 20)
    Library:SetWatermarkVisibility(true)
...
function Library:CreateWindow(...)
    ...
    if type(Config.Footer) == 'table' and #Config.Footer > 0 then
        ...
        for _, Segment in ipairs(Config.Footer) do
            ...
            local Cell = Library:Create(Copyable and 'TextButton' or 'TextLabel', {
                BackgroundTransparency=1, AutomaticSize=Enum.AutomaticSize.X, Size=UDim2.fromOffset(0,18), Text=Text, Font=Library.Font, TextSize=math.max(10,Library.FontSize-2), TextColor3=Library.FontColor, TextXAlignment=Enum.TextXAlignment.Left, AutoButtonColor=false, ZIndex=10, Parent=FooterRow
            })
    ...
    function Window:AddTab(Name, Icon)
        ...
        function Tab:AddPlayerInfo(Idx, Info)
            ...
            Library:Create('ImageLabel',{BackgroundTransparency=1,Size=UDim2.fromOffset(64,64),Position=UDim2.fromOffset(8,8),Image='rbxthumb://type='..((Info.ThumbnailType == 'Bust') and 'AvatarBust' or (Info.ThumbnailType == 'Avatar' and 'Avatar' or 'AvatarHeadShot'))..'&id='..tostring(UserId)..'&w=150&h=150',ZIndex=4,Parent=Holder})
...
function BaseGroupbox:AddPlayerInfo(Idx, Info)
    ...
    Library:AddToRegistry(Holder,{BackgroundColor3='BackgroundColor',BorderColor3='OutlineColor'})
    Library:Create('ImageLabel',{BackgroundTransparency=1,Size=UDim2.fromOffset(math.max(48,Height-20),math.max(48,Height-20)),Position=UDim2.fromOffset(8,10),Image='rbxthumb://type='..((Info.ThumbnailType == 'HeadShot') and 'AvatarHeadShot' or (Info.ThumbnailType == 'Avatar' and 'Avatar' or 'AvatarBust'))..'&id='..tostring(UserId)..'&w=150&h=150',Parent=Holder,ZIndex=6})
    local Desc=Info.Description; if type(Desc)=='table' then Desc=table.concat(Desc,'\n') end
...
function Library:AddWatermark(Segments)
    ...
    elseif type(Segments) == 'table' then
        self:SetWatermarkSegments(Segments)
    else
