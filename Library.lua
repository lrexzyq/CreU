...
function Library:UpdateBlur()
    if Library.UseBlur then
        ...
        task.delay(0.2, function()
            ...
            if Blur and Blur.Parent and not Library.UseBlur and not Library.Unloaded then
                Blur.Enabled = false
            end
        end)
    end
...
function Library:AddToolTip(InfoStr, HoverInstance)
    ...
    HoverInstance.MouseEnter:Connect(function()
        ...
        while IsHovering and not Library.Unloaded and Tooltip.Parent and HoverInstance.Parent do
            RunService.Heartbeat:Wait()
            if not IsHovering or Library.Unloaded or not Tooltip.Parent or not HoverInstance.Parent then
                break
            end
            Tooltip.Position = UDim2.fromOffset(Mouse.X + 15, Mouse.Y + 12)
        end
        if not HoverInstance.Parent then
            IsHovering = false
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        local ModeSelectOuter = Library:Create('Frame', {
            ...
            Size = UDim2.new(0, 92, 0, (#(Info.Modes or { 'Always', 'Toggle', 'Hold' }) * 18) + 2);
            Visible = false;
            ZIndex = 114;
            Parent = ScreenGui;
        });
        ...
        for Idx, Mode in next, Modes do
            ...
            Button.InputBegan:Connect(function(Input)
                ...
                ChangedConn = InputService.InputChanged:Connect(function(Change)
                    if Change ~= Input and not (Change.UserInputType == Enum.UserInputType.MouseMovement and Input.UserInputType == Enum.UserInputType.MouseButton1) then
                        return
                    end
                    if (Change.Position - StartPosition).Magnitude > 6 then
                        Moved = true
                    end
                end)
                EndedConn = InputService.InputEnded:Connect(function(EndInput)
        ...
        function KeyPicker:SetValue(Data)
            ...
            KeyPicker.DisplayUnknown = (Key == 'Touch') or (InputService.TouchEnabled and (Key == 'None' or Key == ''))
            if Key == 'Touch' or Key == '' then
                Key = 'None'
            end
            KeyPicker.Value = Key;
            RefreshKeyDisplay()
        ...
        local function BeginPicking()
            ...
            Event = InputService.InputBegan:Connect(function(Input)
                ...
                Break = true
                Picking = false
                KeyPicker.DisplayUnknown = false
                KeyPicker.Value = Key
                RefreshKeyDisplay()
        ...
        KeyButton.InputBegan:Connect(function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                -- Touch is intentionally not a bindable key. Mobile activation
                -- is handled by the Keybinds list row instead.
                Library:EndGesture(Input)
            end
        end);
...
function Funcs:AddDropdown(Idx, Info)
        ...
        local SearchHeight = Dropdown.Searchable and 24 or 0;
        ...
        function Dropdown:SetValues(NewValues)
            ...
            local NormalizedValues = NewValues
        ...
        function Dropdown:SetSearchable(Value)
            Value = Value == true;
            if Value == self.Searchable then return self end
...
function Library:SetShowKeybinds(Visible)
    ...
    if Library.ShowKeybinds then
        if type(Library.RefreshKeybinds) == 'function' then
            Library:RefreshKeybinds()
        end
    else
        if Library.KeybindFrame then
        ...
        if Library.KeybindContainer then
            for _, Entry in ipairs(Library.KeybindContainer:GetChildren()) do
