...
do
    ...
    function ThemeManager:ApplyTheme(Theme)
        ...
        local UpdateOk = pcall(self.ThemeUpdate, self)
        self._ApplyingTheme = false
    ...
    function ThemeManager:LoadDefault()
        ...
        local Content
        local Result = SafeReadFile(Path); local OkRead = Result ~= nil
        ...
        -- Always apply the selected/default theme itself. Updating only the
        -- dropdown used to leave the library colors on the previous theme.
    ...
    function ThemeManager:SaveDefault(Theme)
        ...
        local Ok, Err = SafeWriteFile(self.Folder .. "/themes/default.txt", Theme)
        if not Ok then
            return false, Err
        end

        -- Setting a theme as the startup default must not change the theme
        -- currently applied to the UI. CurrentTheme is runtime state; this
        -- operation only changes the persisted startup preference.
        return true
    end
    ...
    function ThemeManager:CreateThemeManager(Groupbox)
        ...
        local function UpdateTheme()
            ...
            -- A manual color edit means the current colors are no longer an
            -- exact representation of the named theme. Do not let SaveManager
            -- later restore the old theme and silently overwrite the edit.
    ...
    function ThemeManager:GetCustomTheme(File)
        ...
        local Raw = SafeReadFile(Path); local OkRead = Raw ~= nil
    ...
    function ThemeManager:SaveCustomTheme(File)
        ...
        local OkWrite, Err = SafeWriteFile(Path, Encoded)
    ...
    function ThemeManager:ReloadCustomThemes()
        local List = SafeListFiles(self.Folder .. "/themes"); local Ok = List ~= nil
    ...
    function ThemeManager:SetLibrary(Library)
        ...
        -- Expose the manager through Library so SaveManager can persist/restore
        -- the active theme without requiring a second global connection.
    ...
    function ThemeManager:BuildFolderTree()
        ...
        for _, Path in ipairs(Paths) do
            local Exists = SafeIsFolder(Path)
            if not Exists then
                SafeMakeFolder(Path)
            end
        end
